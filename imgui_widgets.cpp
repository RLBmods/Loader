...
bool ImGui::Tab(const char* icon, const char* label, int* tab, int number)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(160, 40));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, *tab == number ? main_color : hovered ? ImColor(1.f, 1.f, 1.f, 0.2f) : ImColor(0.12f, 0.12f, 0.12f, 0.f), g.IO.DeltaTime * anim_speed);
    it_anim->second.line_offset = ImLerp(it_anim->second.line_offset, *tab == number ? 6.f : -5.f, g.IO.DeltaTime * anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *tab == number ? ImColor(1.f, 1.f, 1.f, 1.f) : ImColor(0.6f, 0.6f, 0.6f, 1.f), g.IO.DeltaTime * anim_speed);

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.background_color), style.FrameRounding);

    PushFont(icon_font);
    window->DrawList->AddText(total_bb.Min + ImVec2(20 - CalcTextSize(icon).x / 2, 20 - CalcTextSize(icon).y / 2), GetColorU32(it_anim->second.text_color), icon);
    PopFont();

    PushFont(tabs_font);
    window->DrawList->AddText(total_bb.Min + ImVec2(40, 20 - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.text_color), label);
    PopFont();
...
bool ImGui::Product(ImTextureID texture_id, const char* label, int* v, int number)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect bb(pos, pos + ImVec2(260, 150));
    ...
    it_anim->second.offset = ImLerp(it_anim->second.offset, hovered ? ImVec2(0.15f, 0.15f) : ImVec2(0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.alpha = ImLerp(it_anim->second.alpha, hovered ? 0.8f : 0.f, g.IO.DeltaTime * 6.f);
    it_anim->second.alpha_image = ImLerp(it_anim->second.alpha_image, hovered ? 1.f : 0.6f, g.IO.DeltaTime * 6.f);
    it_anim->second.text_alpha = ImLerp(it_anim->second.text_alpha, hovered ? 1.f : 0.f, g.IO.DeltaTime * 6.f);

    window->DrawList->AddImageRounded(texture_id, bb.Min, bb.Max, ImVec2(0, 0) + it_anim->second.offset, ImVec2(1, 1) - it_anim->second.offset, ImColor(1.f, 1.f, 1.f, it_anim->second.alpha_image), 4.f);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(0.f, 0.f, 0.f, it_anim->second.alpha), 4.f);

    window->DrawList->AddText(icon_font, 25, bb.Min + ImVec2(10, 10), ImColor(0.9f, 0.9f, 0.9f, 1.f), "a");

    if (pressed)
        *v = number;

    PushFont(big_font);
    window->DrawList->AddText(bb.Min + (bb.Max - bb.Min) / 2 - CalcTextSize(label) / 2, ImColor(1.f, 1.f, 1.f, it_anim->second.text_alpha), label);
    PopFont();

    window->DrawList->AddRect(bb.Min, bb.Max, second_color, 4.f, 0, 3.f);
...
bool ImGui::News(ImTextureID texture_id, const char* label, const char* description, bool *notif_hide)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems || *notif_hide)
        return false;
    ...
    static std::map<ImGuiID, news_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, news_state() });
        it_anim = anim.find(id);
    }

    const ImRect bb(pos, pos + ImVec2(530, 70));
    ItemSize(bb, style.FramePadding.y);
    ItemAdd(bb, id);
    ...
    it_anim->second.button_color = ImLerp(it_anim->second.button_color, held ? ImColor(25, 25, 25, 100) : hovered ? ImColor(15, 15, 15, 100) : ImColor(10, 10, 10, 100), g.IO.DeltaTime * 8.f);

    if (held)
    {
        if (!it_anim->second.status_mouse) {
            it_anim->second.mouse_pos = g.IO.MousePos;
            it_anim->second.status_mouse = true;
        }
    }
    else {
        it_anim->second.status_mouse = false;
        it_anim->second.offset_x = ImLerp(it_anim->second.offset_x, it_anim->second.hiden ? 810.f : 0.f, g.IO.DeltaTime * 3.f);
    }

    if (g.IO.MousePos.x - it_anim->second.mouse_pos.x < 190 && g.IO.MousePos.x - it_anim->second.mouse_pos.x > 0.f && held)
        it_anim->second.offset_x = g.IO.MousePos.x - it_anim->second.mouse_pos.x;

    if (it_anim->second.offset_x > 90 && !held)         
        it_anim->second.hiden = true;

    if(it_anim->second.offset_x > 610 && it_anim->second.hiden)
        *notif_hide = true;


    window->DrawList->AddRectFilled(bb.Min + ImVec2(it_anim->second.offset_x, 0), bb.Max + ImVec2(it_anim->second.offset_x, 0), GetColorU32(it_anim->second.button_color), style.FrameRounding);
    window->DrawList->AddRect(bb.Min + ImVec2(it_anim->second.offset_x, 0), bb.Max + ImVec2(it_anim->second.offset_x, 0), second_color, style.FrameRounding);

    window->DrawList->AddImageRounded(texture_id, bb.Min + ImVec2(10 + it_anim->second.offset_x, 10), bb.Min + ImVec2(90 + it_anim->second.offset_x, 60), ImVec2(0, 0), ImVec2(1, 1), ImColor(1.f, 1.f, 1.f, 1.f), 4.f);

    window->DrawList->AddText(ubuntu_font, 20, bb.Min + ImVec2(100 + it_anim->second.offset_x, 10), ImColor(1.f, 1.f, 1.f, 1.f), label);
    window->DrawList->AddText(bb.Min + ImVec2(100 + it_anim->second.offset_x, 30), ImColor(0.6f, 0.6f, 0.6f, 1.f), description);

    return pressed && it_anim->second.offset_x < 2.f;
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect bb(pos + it_anim->second.size_offset, pos + size - it_anim->second.size_offset);
    ...
    it_anim->second.button_color = ImLerp(it_anim->second.button_color, held ? main_color : hovered ? ImColor(11, 11, 11, 255) : ImColor(10, 10, 10, 255), g.IO.DeltaTime * 8.f);
    it_anim->second.size_offset = ImLerp(it_anim->second.size_offset, held ? size_arg / 30 : hovered ? size_arg / 35 : ImVec2(0, 0), g.IO.DeltaTime * 8.f);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.button_color), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, second_color, style.FrameRounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, IsItemActive() ? ImColor(12, 12, 12, 255) : hovered ? ImColor(11, 11, 11, 255) : ImColor(10, 10, 10, 255), g.IO.DeltaTime * 8.f);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? ImColor(0.7f, 0.7f, 0.7f, 1.f) : ImColor(0.35f, 0.35f, 0.35f, 1.f), g.IO.DeltaTime * 8.f);

    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.background_color), style.FrameRounding);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, second_color, style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max + ImVec2(0, 10.f), ImColor(0.2f, 0.2f, 0.2f, 0.7f));
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            window->DrawList->AddText(ImVec2(frame_bb.Min.x + 15.f + style.ItemInnerSpacing.x, frame_bb.Min.y + size_arg.y / 2 - CalcTextSize(buf_display).y / 2), GetColorU32(it_anim->second.text_color), buf_display);
        }
        ...
        if (render_cursor)
        {
            ...
            it_anim->second.slow_moveX0 = ImLerp(it_anim->second.slow_moveX0, ImVec2(cursor_screen_rect.Min.x, total_bb.Min.y + size_arg.y / 2), g.IO.DeltaTime * 15.f);
    ...
    {
        PushFont(IconFont);
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), ImColor(1.f, 1.f, 1.f, 1.f), "5");
        PopFont();
    }
    ...
    {
        PushFont(IconFont);
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), ImColor(1.f, 1.f, 1.f, 1.f), "1");
        PopFont();
    }
    ...
    {
        PushFont(IconFont);
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), ImColor(1.f, 1.f, 1.f, 1.f), "6");
        PopFont();
    }
    */
    if (label_size.x > 0 && (bool)!buf[0])
    {
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + 15.f + style.ItemInnerSpacing.x, frame_bb.Min.y + size_arg.y / 2 - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.text_color), label);
    }
